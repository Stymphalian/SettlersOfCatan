...
int first_function(lua_State* state){
	if(check_num_of_args(state,2) != 0){
		Logger::getLog().log(Logger::ERROR, "first_function error not enough arguments");
		return 0;
	}	
...
lizz_lua::lizz_lua(){
	Logger::getLog().log(Logger::DEBUG, "lizz_lua::lizz_lua() Constructor");
	this->lua_state = luaL_newstate();
	if(this->lua_state == nullptr){
		this->active = false;
		Logger::getLog().log(Logger::ERROR,"lizz_lua::lizz_lua() Unable to obtain lua_state" );
		return;
	}
...
lizz_lua::~lizz_lua(){
	Logger::getLog().log(Logger::DEBUG, "lizz_lua::~lizz_lua Destructor");
...
bool lizz_lua::run_file(const char* file){
	...
	if(luaL_dofile(this->lua_state, file)){
		Logger::getLog().log(Logger::DEBUG, "lizz_lua::run_file(%s) Failed to run lua script",file);
		return false;
	} 
	return true;
}
...
void lizz_lua::load(){	
	Logger::getLog().log(Logger::DEBUG, "lizz_lua::load() Registering lua functions");
