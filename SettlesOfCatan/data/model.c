...
Model::Model(int num_players){
	...
	if(num_players <= 4){
		...
		m_num_extensions = ((num_players - 4) / 2) + ((num_players % 2 == 1) ? 1 : 0);
		m_num_levels = m_num_extensions + 4;
	}
	Logger::getLog("Jordan.log").log(Logger::DEBUG, "Model Constructor: num_levels=%d,num_extensions=%d", m_num_levels,m_num_extensions);
...
void Model::establish_board_dimensions(int num_players){
	...
	Logger::getLog("Jordan.log").log(Logger::DEBUG, "Model::establish_board_dimensions m_board_width=%d", m_board_width);
...
bool Model::fill_board(Tiles* board, int size,int num_levels){
	...
	int offset = 0;
	int num_tiles = 0;
	int width = 2 * num_levels - 1;	
	int active_count = 0;
	for(int row = 0; row < width; ++row){		
		num_tiles = numtiles_from_row(row, num_levels);
		offset = num_offset_tiles_from_row(row, num_levels);
	...
	logger.log(logger.DEBUG, "Model.fill_board() Setting up the water_tile ordering. num_game_tiles=%d", m_num_water_tiles);
	logger.log(logger.DEBUG, "Model.fill_board() Setting up the roll ordering. num_game_tiles=%d", m_num_game_tiles - m_num_water_tiles);
	...
	for(int row = 0; row < width; ++row){
		for(int col = 0; col < width; ++col){
			...
			if(hex->ring_level == m_num_levels-1){				
				...
				if(water_tile_pos >= water_tile_order_size){ continue; }
				hex->type = water_tile_order[water_tile_pos++];					
				hex->roll = 0;
			} else{
				// place normal tiles
				if(tile_pos >= tile_order_size) { break; }
				if(roll_pos >= roll_order_size) { break; }
...
void Model::shuffle(int* arr, int size){
	if(!arr){ return; }
	Logger::getLog("jordan.log").log(Logger::DEBUG, "Model::shuffle(array,size=%d)", size);
	...
	delete[] copy;
	delete[] taken;
	Logger::getLog("jordan.log").log(Logger::DEBUG, "Model::shuffle rolled %u times for %d items",count,size);
}

bool Model::fill_deck(int* default_cards, int size){
	if(!default_cards){ return false; }
	Logger& logger = Logger::getLog("Jordan.log");
	logger.log(logger.INFO,"Model::fill_deck(default_cards,size=%d)", size);
	...
	shuffle(victory_titles, victory_title_size);
	shuffle(random_cards, random_cards_size);
...
int Model::get_ring_level(int row, int col,int levels){
	...
	int dy = abs(levels - 1 - row);
	int numtiles = numtiles_from_row(row,levels);
	int offset = num_offset_tiles_from_row(row,levels);
	int border_len = numtiles - levels;
	int center = offset + numtiles / 2;
	int l = abs(center - offset);
	int t = l - border_len;
	if(numtiles % 2 == 0 && col >= center){
		l--; t--;
	}
	int x = abs(center - col);	
...
int Model::num_offset_tiles_from_row(int row, int level){
	int offset = level_from_row(row, level)/2;
	if(row % 2 == 0){
		if(level % 2 == 0){ offset++; }
	}
	return offset;
...
int Model::level_from_row(int row, int levels){
	return abs(levels - 1 - row);
}

// Tells you how many tiles there are on this row given the number of levels there are
int Model::numtiles_from_row(int row, int level){
	return 2 * level - 1 - level_from_row(row, level);
}
...
bool Model::get_dev_card(int player){
	// pay for the card
...
bool Model::fill_vertex_face_arrays(){
	Logger::getLog("jordan.log").log(Logger::DEBUG, "Model::fill_vertex_face_arrays() filling the face and vertex arrays");
	Tiles* hex;
	Tiles* other = 0;
	vertex_face_t* t;	
	int x, y;
	...
	int count;
	logger.log(Logger::DEBUG, "vertex array");
	count = 0;
	for(it = vertex_array.begin(); it != vertex_array.end(); ++it){
		vertex_face_t* t = &(*it);
		logger.log(Logger::DEBUG, "vertex[%d] faces {%d,%d,%d} vertices {%d,%d,%d} tiles{ (%d,%d), (%d,%d), (%d,%d)}",
			count,
	...
	logger.log(Logger::DEBUG, "face array");
	count = 0;
	for(it = face_array.begin(); it != face_array.end(); ++it){
		vertex_face_t* t = &(*it);
		logger.log(Logger::DEBUG, "face[%d] faces {%d,%d,%d,%d} vertices {%d,%d} tiles{ (%d,%d), (%d,%d)}",
			count,
...
bool Model::pay_for_item(int player, resource_t* price){
	...
	return bank_exchange(player, price,&take);
}
...
bool Model::bank_exchange(int player, resource_t* give, resource_t* take){
	...
	if(good_flag == false){ return false; }
...
bool Model::move_thief(int col, int row){
	...
	if(m_board[col + row*m_board_width].type >= Tiles::WATER_TILE){
		return false;
	}
	/*	
	if(get_ring_level(row, col, m_num_levels) == m_num_levels - 1){
		return false;
	}
	*/
	thief_pos_x = col;
